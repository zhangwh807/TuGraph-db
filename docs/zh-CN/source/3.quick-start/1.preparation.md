...
**方式二**
   ...
   # 命令将数据目录和日志目录挂载到了宿主机的/root/tugraph/上进行持久化，您可以根据实际情况修改。
   ```
请注意，由于大版本之间存在存储不兼容问题，在进行跨大版本升级时，如果继续使用旧版本宿主机上的持久化目录，会导致TuGraph服务无法启动。为避免此问题，请在跨大版本升级时更改持久化目录的路径。如果希望在新版本中使用之前的数据，请参考[数据迁移](../13.best-practices/3.data_migration.md)章节中的"升级迁移"章节。首次安装和部署则不受此问题影响。
